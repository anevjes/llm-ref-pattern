...
//
// Notes:
// - Resource tags applied: Environment, Project, ManagedBy, LastDeployed.
// - Resource naming follows CAF pattern where applicable: {resource-type-prefix}-{workload}-{environment}.
// - Diagnostic settings are enabled where supported.
// ============================================================================
...
@description('Tags to apply to all resources. Defaults are merged with Environment, Project, ManagedBy, LastDeployed.')
param tags object = {}
...
resource routeTable 'Microsoft.Network/routeTables@2024-01-01' = {
  ...
  properties: {
    ...
    routes: [
      {
        name: 'rt-fw-${workloadName}-${environment}'
        properties: {
...
// ---------------------------------------------------------------------------
// Hub Virtual Network (central connectivity VNet)
// ---------------------------------------------------------------------------
resource hubVnet 'Microsoft.Network/virtualNetworks@2024-01-01' = {
  name: hubVnetName
  location: location
  // Tags follow CAF recommendation and include required governance tags
  tags: union(defaultTags, {
    Environment: environment
    Project: projectName
    ManagedBy: 'bicep'
  ...
  properties: {
    ...
    subnets: [
      ...
      {
        // CAF-compliant subnet name: snet-hub-mgmt-<env> already aligned via prefix
        name: 'snet-hub-mgmt'
        properties: {
...
resource spokeVnet 'Microsoft.Network/virtualNetworks@2024-01-01' = {
  ...
  properties: {
    ...
    subnets: [
      {
        // CAF-compliant subnet for application workloads
        name: 'snet-app'
        properties: {
      ...
      {
        // CAF-compliant subnet for data services
        name: 'snet-data'
        properties: {
      ...
      {
        // CAF-compliant subnet for integration components
...
// Diagnostic settings for Spoke Virtual Network
...
// Hub-to-Spoke VNet peering (CAF: peer-<workload>-<env>)
...
resource sqlDtuAlert 'Microsoft.Insights/metricAlerts@2018-03-01' = {
  name: 'mal-sql-${workloadName}-${environmentName}-dtu-${sqlDatabaseName}'
  location: 'global'
  tags: union(defaultTags, {
    Environment: environmentName
